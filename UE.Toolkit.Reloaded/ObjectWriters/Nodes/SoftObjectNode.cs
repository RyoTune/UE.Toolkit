...
public class SoftObjectNode(IFProperty property, Ptr<FSoftObjectPtr> value) 
    : TextPrimitiveNode<IFProperty, FSoftObjectPtr>(property, value)
{
    protected override unsafe void SetField(string text)
    {
        ...
        Log.Debug($"{nameof(SoftObjectNode)} || Field '{property.NamePrivate}' at 0x{Value:X} set to: {text}");
    }
