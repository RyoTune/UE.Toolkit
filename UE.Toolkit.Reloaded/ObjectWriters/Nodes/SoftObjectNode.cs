...
public class SoftObjectNode(IFProperty property, Ptr<FSoftObjectPtr> value, IUnrealClasses classes) 
    : TextPrimitiveNode<IFProperty, FSoftObjectPtr>(property, value)
{
    ...
    protected override unsafe void SetField(string text)
    {
        ...
        Log.Debug($"{nameof(SoftObjectNode)} || Field '{Property.NamePrivate}' at 0x{(nint)Value.Value:X} set to '{text}'");
    }
